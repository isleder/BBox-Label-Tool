...
class LabelTool():
    def __init__(self, master, imgdir, labeldir):
        ...
        self.parent.bind("[", self.prevImage) # press 'a' to go backforward
        self.parent.bind("]", self.nextImage) # press 'd' to go forward
...
##                    print tmp
                    ...
                    self.listbox.insert(END, '%d,%d %d,%d w%d h%d' %(tmp[0], tmp[1], tmp[2], tmp[3], tmp[2]-tmp[0], tmp[3]-tmp[1]))
                    self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])

    def saveImage(self):
        if len(self.bboxList) == 0: return
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            self.bboxId = None
            self.listbox.insert(END, '%d,%d %d,%d w%d h%d' %(x1, y1, x2, y2, x2-x1, y2-y1))
            self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])
